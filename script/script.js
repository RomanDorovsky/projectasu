...
const renderTri1 = () => {
    ...
    <img class="img" src="img/4.png"></div>
    </div>`
};
...
document.getElementById('t-menu3').onclick = function() {
    setTimeout(contentHide,0)
    setTimeout(contentUnhide,1200)
    setTimeout(renderTri1,1000)
}

const renderTri2 = () => {
    page.innerHTML = `<div class="txt">
    <div class="txt-header">Эмоции персонажей</div>
    <div class="display-flex"><div><p>Вспомните пять известных исторических личностей и сфокусируйтесь на их эмоциональном состоянии в различные моменты жизни.</p>
    <p>Приведем примеры: Махатма Ганди. Борьба за независимость Индии и мирное сопротивление Ганди исходили из его глубоких убеждений в ненасилии и праведности. В то же время он мог чувствовать усталость от непрерывной борьбы, решимость в достижении своих целей и сострадание к страдающим.</p>
    <p>Или, Василий IV: Судьба царевича Василия IV была полна испытаний и борьбы за власть во времена Смутного времени. Он мог чувствовать страх за свою жизнь, обиду от неправедных обвинений в измене, но тем не менее сохранять надежду на будущее и веру в свою справедливость.</p></div>
    <img class="img" src="img/5.png"></div>
    </div>`
};
...
document.getElementById('t-menu4').onclick = function() {
    setTimeout(contentHide,0)
    setTimeout(contentUnhide,1200)
    setTimeout(renderTri2,1000)
}

const renderTri3 = () => {
    page.innerHTML = `<div class="txt">
    <div class="txt-header">Эмпатия и злость</div>
    <div class="display-flex"><div><p>Упражнение научит вас применять эмпатию для преодоления чувства злобы к другому человеку. Представьте себе ситуацию, когда вы очень сердились на кого-то, а затем сформулируйте эмпатическое высказывание. Например:</p>
    <p>Раздраженный человек: "Ты никогда не выполняешь то, о чем я прошу!"</p>
    <p>Эмпатический ответ: "Ты чувствуешь раздражение потому, что я не выполнил свои обязанности, и тебе пришлось делать мою работу вместо меня."</p></div>
    <img class="img" src="img/6.png"></div>
    </div>`
};
...
document.getElementById('f-menu2').onclick = function() {
    setTimeout(contentHide,0)
    setTimeout(contentUnhide,1200)
    setTimeout(renderF,1000)
}

const renderF1 = () => {
    page.innerHTML = `<div class="txt">
    <div class="txt-header">Найти и запомнить</div>
    <div class="display-flex"><div><p>В течение дня уделите внимание десяти моментам радости, позвольте себе искренне насладиться ими и заметьте, как изменится ваше настроение. Если получится, взгляните на себя в зеркало, почувствуйте эмоции и постарайтесь сохранить в памяти свои физические ощущения.</p>
    <p>На следующий день обнаружьте десять моментов, вызывающих удивление, и отследите, как это повлияет на ваше состояние.</p>
    <p>По аналогичной схеме попрактикуйтесь вызывать и осознавать все эмоции, способствующие повышению эффективности человеческой деятельности: принятие, доверие, интерес, восхищение.</p></div>
    <img class="img" src="img/7.png"></div>
    </div>`
};
...
document.getElementById('f-menu4').onclick = function() {
    setTimeout(contentHide,0)
    setTimeout(contentUnhide,1200)
    setTimeout(renderF2,1000)
}

const renderF3 = () => {
    page.innerHTML = `<div class="txt">
    <div class="txt-header">Главное - дышать</div>
    <div class="display-flex"><p>Ещё один способ держать эмоции под контролем – это выработать автоматический рефлекс, состоящий из глубокого вдоха и медленного выдоха. Рефлекс вырабатывается регулярным выполнением упражнения – вспомните какую-нибудь негативную эмоцию (например, вспоминаете как вас обругал учитель), и тут же делаете глубокий вдох и медленный выход. Если делать такое упражнение три раза в день и несколько месяцев – это будет происходить на автомате при проживании негативных эмоций.</p>
    <img class="img" src="img/9.png"></div>
    </div>`
};

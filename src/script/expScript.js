        ...
        if(DATA.length === nextTextIndex){
            ...
            if (Number(header.dataset.currentStep) === 7){
                ...
                if (choiceBtn1.checked === true){
                    expbg.innerHTML = `<p class="exp-text"> — А, Лена. Не то чтобы мне было до этого дело, но в целом, посмотрим что она будет делать дальше<br>Правда, особо роли это не сыграло, так как моё дальнейшее предложение насчёт того, чтобы самим заработать на поездку всё равно было поддержано всем классом, за исключением Вальки-живодёра. Кстати об этом...</p>`
                }
                else if (choiceBtn2.checked === true){
                    expbg.innerHTML = `<p class="exp-text"> — Перед кем? Да перед ней-то? Ага, разбежался.<br>Правда, особо роли это не сыграло, так как моё дальнейшее предложение насчёт того, чтобы самим заработать на поездку всё равно было поддержано всем классом, за исключением Вальки-живодёра. Кстати об этом...</p>`
                }
                else if (choiceBtn3.checked === true){
                    expbg.innerHTML = `<p class="exp-text"> — Перед ней? Пожалуй, а почему бы и нет? Быть рыцарем - вообще отлично.<br>Правда, особо роли это не сыграло, так как моё дальнейшее предложение насчёт того, чтобы самим заработать на поездку всё равно было поддержано всем классом, за исключением Вальки-живодёра. Кстати об этом...</p>`
                }
                btnNext.addEventListener('click', (event) => {
